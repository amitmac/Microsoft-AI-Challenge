...
logging.basicConfig(filename="data/test_logs.log", level=logging.INFO)
...
flags.DEFINE_bool("do_train", False, "training the model?")
...
flags.DEFINE_integer("predict_batch_size", 16, "prediction batch size")
flags.DEFINE_integer("num_labels", 6, "number of classes")
flags.DEFINE_float("learning_rate", 5e-5, "The initial learning rate for Adam")
flags.DEFINE_float("num_epochs", 1.0, "number of epochs")
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, labels,
    ...
    pooled_output_layer = model.get_pooled_output()

    ques_outputs = sequence_outputs[:,:FLAGS.max_ques_length,:]
    para_outputs = sequence_outputs[:,FLAGS.max_ques_length:,:]    

    hidden_size = sequence_outputs.shape[-1].value
    para_num_tokens = FLAGS.max_sequence_length - FLAGS.max_ques_length

    attention_weights = tf.get_variable("att_weights", [hidden_size, para_num_tokens],
                                        initializer=tf.truncated_normal_initializer(stddev=0.02))

    ques_outputs = tf.reshape(ques_outputs, [-1, hidden_size])
    
    ques_para_context = tf.reshape(tf.matmul(ques_outputs, attention_weights), 
                                    [-1, FLAGS.max_ques_length, para_num_tokens])
    
    attention_vector = tf.reduce_mean(ques_para_context, axis=1)

    output_layer = tf.multiply(tf.reshape(attention_vector, [-1, para_num_tokens, 1]), para_outputs)

    para_mask = tf.reshape(tf.cast(input_mask[:,FLAGS.max_ques_length:], tf.float32), 
                           [-1, para_num_tokens, 1])
    output_layer = tf.reduce_mean(tf.multiply(output_layer, para_mask), axis=1)

    output_layer = tf.concat([output_layer, pooled_output_layer], axis=1)
...
def create_cnn_attention_model(bert_config, is_training, input_ids, input_mask, segment_ids, labels,
    ...
    ques_outputs = sequence_outputs[:,:FLAGS.max_ques_length,:] # B x QT x H
    para_outputs = sequence_outputs[:,FLAGS.max_ques_length:,:] # B x PT x H
    para_num_tokens = FLAGS.max_sequence_length - FLAGS.max_ques_length
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        if not os.path.exists(predict_file):
            predict_data = data_ops.get_data(FLAGS.data_dir + "eval1_unlabelled.tsv", test=True)
